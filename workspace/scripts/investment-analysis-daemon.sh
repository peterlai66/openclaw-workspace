...
LOG_FILE="/home/pclaw/.openclaw/workspace/logs/investment-analysis.log"
PID_FILE="/tmp/investment-analysis.pid"
...
if [ -f "$PID_FILE" ]; then
    ...
    if kill -0 "$pid" 2>/dev/null; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') - 投資分析系統已在運行" >> "$LOG_FILE"
        exit 0
    fi
fi
